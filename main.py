...
def main(args):
    ...
    # Creates ckpts_asap folder with model name folder inside if not created 
    ...
    # Creates new argument called save_model_path ckpts_asap/t5-base 
    args.save_model_path = f"ckpts_{args.result_path}"


    # if in test mode it modifies args.load_checkpoint_path to point to a checkpoint directory where model checkpoints are saved
    if args.test:
    ...
    """
    dropout=0.1  # Recommended for ASAP unless underfitting

    How to Test?
    First run: Train without dropout, monitor validation loss.

    If it overfits (val loss ↑ after epochs), add dropout=0.1.

    Second run: Train with dropout, check if validation improves.

    Tweak to 0.05 (if too aggressive) or 0.2 (if still overfitting).
    ...
    # Add an adapter to the T5 model
    adapter_name = "multi_trait_scoring"



    # Define AdapterConfig and add it to the model
    adapter_config = AdapterConfig.load(
    "pfeiffer",  # or "houlsby"
    reduction_factor=16,  # Controls bottleneck size (e.g., 16 reduces d_model=768 → 48).
    non_linearity="gelu", 
)
 

    model.add_adapter(adapter_name, config=adapter_config)
    model.train_adapter(adapter_name)  # Freeze base model, train only adapters
